...
let input = {
  ...
  assertEq(result.value.longer, 1, "First value for 'longer' should be 1");
  assertEq(result.value.shorter, 'a', "First value for 'shorter' should be 'a'");
  
  result = iterator.next();
  assertEq(result.done, false, "Iterator should not be done after second next()");
  assertEq(result.value.longer, 2, "Second value for 'longer' should be 2");
  assertEq(result.value.shorter, 'b', "Second value for 'shorter' should be 'b'");
