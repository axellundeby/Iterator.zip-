...
assertEq(result.done, true, "Iterator.zipKeyed on an object with no own-enumerable keys should be finished");

// Even subsequent calls should return done:true.
result = iter.next();
assertEq(result.done, true, "Subsequent next() calls on a finished iterator should still be finished");
