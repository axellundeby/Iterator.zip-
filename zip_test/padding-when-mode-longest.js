...
assertDeepEq(result.value, [2, 'Y', false], "Second zipped value should use explicit padding");
...
result = iterator.next();
assertDeepEq(result.value, [2, 'PAD', false], "Second zipped value should pad iter2 with 'PAD'");
assertEq(result.done, false, "Iterator should not be done after second next()");

result = iterator.next();
assertDeepEq(result.value, [3, 'PAD', undefined], "Third zipped value should pad iter3 with undefined");
assertEq(result.done, false, "Iterator should not be done after third next()");
...
result = iterator.next();
assertDeepEq(result.value, [2, 'b', undefined], "Second zipped value should pad iter3 with undefined");
assertEq(result.done, false, "Iterator should not be done after second next()");

result = iterator.next();
assertDeepEq(result.value, [undefined, 'c', undefined], "Third zipped value should pad iter1 and iter3 with undefined");
assertEq(result.done, false, "Iterator should not be done after third next()");
