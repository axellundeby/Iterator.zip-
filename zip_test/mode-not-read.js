...
let invalidIterables = [
  ...
  for (let value of invalidIterables) {
    ...
    assertThrowsInstanceOf(TypeError, () => Iterator.zip(value, options), 
      `Expected TypeError for first argument: ${String(value)}`);
    assertEq(optionsGetterCalled, false, "Mode should not be accessed if first argument is invalid");
  }
