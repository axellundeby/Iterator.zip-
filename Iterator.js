...
function IteratorZip(iterables, options = {}) {
  ...
  while (!next.done) {
    ...
    if (!next.done) {
      ...
      var iter = GetIterator(next.value, false);
  ...
  if (mode === 'longest') {
    ...
    //b. Else
      ...
      if (usingIterator) {
        ...
        var completion = IteratorClose(paddingIter, NormalCompletion(undefined));
...
function zipping(iters, mode, padding, finishResults) {
  ...
  function closure() {
    ...
    // (we assume openIters always matches length, no assert needed here)
    ...
    while (i < iterCount) {
      ...
      if (iter === null) {
        ...
        // 3.b.iii.3.a. result = Completion(IteratorStepValue(iter))
        var result;
        try {
        ...
        if (result.done) {
          ...
          // strict mode: verify all other iterators are also done
          if (mode === "strict") {
            if (i !== 0) {
              return IteratorCloseAll(
                openIters,
          ...
          // longest:
          // Set iters[i] = null, result = padding[i]
          iters[i] = null;
          results[i] = padding[i];

          // If openIters is now empty, return
          var allClosed = true;
          var j = 0;
          while (j < iterCount) {
            if (openIters[j] !== null) {
...
// const c = Iterator.zip([[1, 2, 3], ['a', 'b']], { mode: 'shortest' });

//const l = Iterator.zip([[1, 2, 3], ['a', 'b']], { mode: 'longest', padding: [1] });

//const s = Iterator.zip([[1, 2], ["a", "b", "c"]], { mode: "strict" });
...
function IteratorCloseAll(iters, completion) {
  ...
  while (i >= 0) {
    ...
    // Spec assumes valid Iterator Record, but we guard nulls
    if (iter !== null && IsObject(iter)) {
      try {
        completion = IteratorClose(iter, completion);
      } catch (e) {
        // If IteratorClose throws, rethrow immediately per `?`
        throw e;
      }
    }
 ...
 */
function IteratorZipKeyed(test) {
  return IteratorCloseAll(test, CreateIteratorResultObject(undefined, true))
}
